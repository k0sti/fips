...
#
# Goal: only FIPS UDP transport (port 4000) may use eth0.
...
iptables -A OUTPUT -o eth0 -p udp --dport 4000 -j ACCEPT
iptables -A OUTPUT -o eth0 -p udp --sport 4000 -j ACCEPT
iptables -A INPUT  -i eth0 -p udp --dport 4000 -j ACCEPT
iptables -A INPUT  -i eth0 -p udp --sport 4000 -j ACCEPT
